...
#define BEACON_UUID "7fd32ed8-9368-4f9d-84f2-fea2107b2539" // UUID 1 128-Bit (may use linux tool uuidgen or random numbers via https://www.uuidgenerator.net/)
...
void loop() {
  ...
  if (digitalRead(Mode_Button)) // Continuous Transmitting Mode (Consumes more battery)
  {
    ...
    BLEDevice::init("ESP32B Beacon Keychain"); // Name of your Beacon Device 
  ...
  else  // Transmit on Trigger Mode
  {
    ...
    // Create the BLE Device
    BLEDevice::init("ESP32B Beacon Keychain");
    // Create the BLE Server
    BLEServer *pServer = BLEDevice::createServer(); // <-- no longer required to instantiate BLEServer, less flash and ram usage
    pAdvertising = BLEDevice::getAdvertising();
    BLEDevice::startAdvertising();
